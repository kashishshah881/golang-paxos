...
func parseLeaderRequest(w http.ResponseWriter, r *http.Request) {
	...
	if newResp.Type == "response" {
		...
		for sname, svalue := range serverlstt {
			...
			fmt.Println(sname+" Processed "+strconv.Itoa(binary.Size(response))+" chunks of bytes!")
...
func main() {
	...
	flag.StringVar(&port,"p",":8004","Enter Port Number. Format:- ':<port_number>'")
	...
	flag.StringVar(&server_name,"server_name","Server04","Enter a Unique Servername with no spaces.")
	flag.Parse()
	RegisterService()
