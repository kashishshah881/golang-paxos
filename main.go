...
func parseLeaderRequest(w http.ResponseWriter, r *http.Request) {
    ...
    len_serverlstt := len(serverlstt)
    
	if newResp.Type == "response" {
        dataCounter := 1
        req_wg.Add(len_serverlstt)
        for sname,svalue := range serverlstt {
            //var newRequest3 request 
            fmt.Println("Sending request to " +sname)
            mr := makeRequest(svalue,data_arr,dataCounter)
            //err := json.Unmarshal(mr,newRequest3)
            if err != nil {
                fmt.Println("Error in unmarshalling line 313")
            }
            //fmt.Println("servername "+sname+" svalue "+ svalue + data_arr[dataCounter] )
            dataCounter++
            w.Write(mr)
        }
...
func makeRequest(severaddress string ,array map[int]string, counter int)[]byte{
    ...
    re,err := http.NewRequest("POST","http://"+severaddress+"/wordcount",bytes.NewBuffer(b))
    resp, err := client.Do(re)
...
func healthyServers(){
    ...
    for server := range servers{
        ...
        if err != nil {
            fmt.Println("url in wrong format")
